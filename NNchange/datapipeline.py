...
class DataPipeline(object):
    ...
    def get_batch(self, fold='train', enable_all_appliances=False,
	    ...
	    batch_ori=batch.before_processing
    ...
    def train_generator(self, fold='train', enable_all_appliances=False,
                  ...
                  validation=False ):
        while 1:
            batch_iter,batch_ori  = self.get_batch(fold, enable_all_appliances, source_id, reset_iterator,validation)
    ...
    def train_generator_ae(self, fold='train', enable_all_appliances=False,
                  source_id=None, reset_iterator=False,
                  validation=False ):
        while 1:
            batch_iter,batch_ori  = self.get_batch(fold, enable_all_appliances, source_id, reset_iterator,validation)
            X_train = batch_iter.input
            input_dim = X_train.shape[1]
            Y_train = batch_iter.target
            target_dim = Y_train.shape[1]
            yield (np.reshape(X_train, [self.num_seq_per_batch, input_dim, 1]),np.reshape(Y_train, [self.num_seq_per_batch, target_dim, 1]))
    def train_generator_z(self, fold='train', enable_all_appliances=False,
                  source_id=None, reset_iterator=False,
                  validation=False ):
        while 1:
            batch_iter,batch_ori = self.get_batch(fold, enable_all_appliances, source_id, reset_iterator,validation)
            X_train = batch_iter.input
            X_train_ori = batch_ori.input
            input_dim = X_train.shape[1]
            Y_train_ae = batch_iter.target
            target_dim = Y_train_ae.shape[1]
            #Y_train_ae_ori = batch_ori.target
            Y_train_reg = self._get_output_neurons(batch_iter)
            #Y_train_reg_ori = self._get_output_neurons(batch_ori)
            yield (np.reshape(X_train, [self.num_seq_per_batch, input_dim, 1]),np.reshape(X_train_ori, [self.num_seq_per_batch, input_dim, 1]),
		   np.reshape(Y_train_ae, [self.num_seq_per_batch, target_dim, 1]),Y_train_reg.astype(np.float32))
	    #yield (np.reshape(X_train, [self.num_seq_per_batch, input_dim, 1]),np.reshape(X_train_ori, [self.num_seq_per_batch, input_dim, 1]),
		   #np.reshape(Y_train_ae, [self.num_seq_per_batch, target_dim, 1]),np.reshape(Y_train_ae_ori, [self.num_seq_per_batch, target_dim, 1]),
		   #Y_train_reg.astype(np.float32),Y_train_reg_ori.astype(np.float32))
            #yield (np.reshape(X_train, [self.num_seq_per_batch, input_dim, 1]),np.reshape(Y_train, [self.num_seq_per_batch, target_dim, 1]))
